...
- [x] `EmailVerification` (token com `Purpose=EmailVerification`, vinculado ao `TargetValue`): ao consumir, marca
      ...
      Entregue como `RequestEmailVerification` (emite e retorna payload de entrega pós-commit) +
...
- [x] Modelar `UserAccountPhone` + verificação + projeções `phone_number`/`phone_number_verified`
      ...
      Entregue como `RequestPhoneVerification` (retorna payload de entrega pós-commit) /
...
- `RoyalIdentity.UserAccounts/Features/Accounts/UseCases/RequestEmailVerification.cs` +
  `ConfirmEmailVerification.cs` — emite token vinculado ao email normalizado e retorna payload de entrega pós-commit /
  consome e marca `IsVerified` do **alvo**.
...
- `RoyalIdentity.UserAccounts/Features/Accounts/UseCases/EmailVerificationRequestResult.cs` +
  `PhoneVerificationRequestResult.cs` — resultados dos requests com payload opcional de entrega.
...
- **Entrega pós-commit**: `RequestEmailVerification`/`RequestPhoneVerification` retornam payload opcional de entrega no
  resultado do handler. O gateway real fica na borda confiável, após persistência da unidade de trabalho.
...
**Correção pós-review da Fase 6 (2026-06-25):** requests de email/telefone passaram a retornar payload de entrega
pós-commit; projeções `phone_*` ficaram inertes quando `EnablePhoneNumber=false`, inclusive na validação de claim types;
índice parcial de telefone primário foi adicionado nos providers SQLite/PostgreSQL. Verificação focada:
`dotnet test Tests.UserAccounts` (158/158) e
`dotnet test Tests.Architecture` (15/15).
