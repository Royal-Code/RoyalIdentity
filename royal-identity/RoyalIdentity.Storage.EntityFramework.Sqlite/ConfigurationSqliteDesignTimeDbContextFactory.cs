...
/// (plan DF11). The history table comes from the centralized topology so scaffolding, the runner and the
/// fixtures cannot disagree (Operational plan DF23).
/// </summary>
    ...
    : IDesignTimeDbContextFactory<ConfigurationSqliteDbContext>
{
    public ConfigurationSqliteDbContext CreateDbContext(string[] args)
    {
        var options = new DbContextOptionsBuilder<ConfigurationSqliteDbContext>()
            .UseSqlite("DataSource=design-time.db", sqlite => sqlite.UseConfigurationMigrationsHistory())
            .Options;
