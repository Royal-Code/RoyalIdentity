<#
.SYNOPSIS
Runs Configuration storage tests against an ephemeral PostgreSQL 17 container managed by Podman.

.DESCRIPTION
Verifies Podman, starts the selected machine when necessary, publishes PostgreSQL on a dynamically allocated
non-default host port, runs the opt-in Configuration migration/seed test, and removes the container in finally.
...
$containerName = "royalidentity-configuration-pg-$PID-$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())"
...
$database = "royalidentity_configuration_tests"
$username = "royalidentity_configuration_tests"
...
try {
	...
	Write-Host "PostgreSQL is healthy on dynamic host port $hostPort. Running Configuration tests..."
	Push-Location $repositoryRoot
	try {
		& dotnet test "Tests.Storage/Tests.Storage.csproj" --filter "Category=PostgreSql"
		if ($LASTEXITCODE -ne 0) {
			$testExitCode = $LASTEXITCODE
			$logs = Invoke-Podman -Arguments @("logs", $containerName)
			throw "Configuration PostgreSQL tests failed with exit code $testExitCode. Container logs:`n$($logs -join [Environment]::NewLine)"
		}
	}
	finally {
		Pop-Location
