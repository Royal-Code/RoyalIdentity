<#
.SYNOPSIS
Runs the Operational storage tests against an ephemeral PostgreSQL 17 container managed by Podman.
...
param(
    ...
    [string] $Filter = "Category=PostgreSql",
...
$containerName = "royalidentity-operational-pg-$PID-$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())"
...
$database = "royalidentity_operational_tests"
$username = "royalidentity_operational_tests"
...
try {
    ...
    Write-Host "PostgreSQL is healthy on dynamic host port $hostPort. Running storage tests (filter: $Filter)..."
    Push-Location $repositoryRoot
    try {
        & dotnet test "Tests.Storage/Tests.Storage.csproj" --filter $Filter
        if ($LASTEXITCODE -ne 0) {
            $testExitCode = $LASTEXITCODE
            $logs = Invoke-Podman -Arguments @("logs", $containerName)
            throw "Operational PostgreSQL tests failed with exit code $testExitCode. Container logs:`n$($logs -join [Environment]::NewLine)"
        }
    }
    finally {
        Pop-Location
