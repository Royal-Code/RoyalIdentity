...
- [.ai/plans/plan-localization.md](.ai/plans/plan-localization.md) — COMPLETED (7/7). `RealmOptions.Internationalization`
...
- [.ai/plans/plan-rfc9700-security-hardening.md](.ai/plans/plan-rfc9700-security-hardening.md) — IN PROGRESS
  (3/6; Fase 4 next). Fase 1 introduced pure ephemeral `ClientSecurityAssessment.Create(Client, RealmOptions)`
  ...
  shared by authorize, code redemption and end-session. Fase 3 made authorization code + PKCE the only
  interactive flow, removed implicit/hybrid/front-channel token issuance, and emits/advertises realm-aware `iss`
  per RFC 9207. Q1 is closed at a 30-second default refresh-token retry tolerance for Fase 4.
