...
- [.ai/plans/plan-users-accounts-sqlite-hardening.md](.ai/plans/plan-users-accounts-sqlite-hardening.md) — IN PROGRESS (backing hardening toward replacing the in-memory fake per ADR-018. All open questions decided. Fase 1 concurrency **retry** DONE: `[WithRetryOnConcurrency]` on the pure-mutation credential use cases, scoped manual retry (`IWorkContext.RetryOnConcurrencyAsync`) on the two token flows, `AuthenticateLocalCredential` excluded (Q4), exhaustion mapped to a fixed `user_account.concurrency_conflict` typeId, `ConcurrencyTests` rewritten against the real handlers with genuine EF-identity-map conflicts. Fase 2 provider migrations (`.Sqlite`/`.PostgreSql`, today only `EnsureCreated`) pending; Fase 3 reusable module seed + module as test backing pending)
